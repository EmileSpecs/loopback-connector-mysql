...
function mixinMigration(MySQL, mysql) {
  ...
  MySQL.prototype.alterTable = function(model, actualFields, actualIndexes, done, checkOnly) {
    ...
    aiNames.forEach(function(indexName) {
      ...
      // Modification to remove the foreign keys (must be here before it's index is removed)
      if (indexName.indexOf('_idx') !== -1) {
        sql.push('DROP FOREIGN KEY ' + self.client.escapeId(indexName.replace('_idx', '')));
      }
    ...
    // add foreign keys according to model relations
    var relations = m.settings.relations;
    var addSql = [];
    if (relations) {
      for(var name in relations) { 
        ...
        if (relations.hasOwnProperty(name)) {
          ...
          if (relation.type == 'belongsTo') {
            var pk = 'id';
            var parent = this.getModelDefinition(relation.model).properties;
            
            for(var prop in parent) { 
              if (parent.hasOwnProperty(prop)) {
                var properties = parent[prop];
                
                if (properties.id) pk = prop;
              }
            }
            addSql.push('ADD INDEX ' + self.client.escapeId(model.toLowerCase() + '_' + name + '_idx') + 
            ' (' + self.client.escapeId(relation.foreignKey) + ' ASC)');
            addSql.push('ADD CONSTRAINT ' + self.client.escapeId(model.toLowerCase() + '_' + name) +
            ' FOREIGN KEY (' + self.client.escapeId(relation.foreignKey) + ')' + 
            ' REFERENCES ' + self.client.escapeId(relation.model) + ' (' + self.client.escapeId(pk) + ')' +
            ' ON DELETE ' + (relation.cascadeDelete ? 'CASCADE' : 'SET NULL') + ' ON UPDATE RESTRICT');
          }
        }
      }
    }

    if (sql.length || addSql.length) {
      var query = 'ALTER TABLE ' + self.tableEscaped(model) + ' ' +
        sql.join(',\n');
        
      // Adding our foreign key SQL to a new ALTER TABLE call because we need the usual SQL code to execute before we add foreign keys
      var addQuery = 'ALTER TABLE ' + self.tableEscaped(model) + ' ' +
        addSql.join(',\n');
      ...
      if (checkOnly) {
        ...
        async.auto({
          table: function(cb) {
            if (!sql.length) return cb();
            self.execute(query, cb);
          },
          relationships: ['table', function(cb) {
            if (!addSql.length) return cb();
            self.execute(addQuery, cb);
          }]
        }, done);
      }
    } else {
